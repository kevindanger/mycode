...
## Define our new fuction called bootstrapper and the expected arugments (all five)
def bootstrapper(dev_type, dev_ip, dev_un, dev_pw, config):
    try:
        ...
        open_connection.send_command_expect('write memory') # write the memory (okay if this gets done
        open_connection.disconnect() # close the open connection

        return True # Everything work! - "Return TRUE when complete"
    except:
